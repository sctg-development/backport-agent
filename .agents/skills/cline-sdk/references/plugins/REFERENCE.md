...
A Cline plugin is a TypeScript module that extends any agent built on the Cline SDK. The same plugin runs in the Cline CLI, VS Code and JetBrains extensions, and any custom app built on `@sctg/cline-core`.
...
```typescript
import type { AgentPlugin } from "@sctg/cline-core"
import { createTool } from "@sctg/cline-core"
...
```typescript
import { type AgentPlugin, ClineCore, createTool } from "@sctg/cline-core"
...
- `peerDependencies` for `@sctg/cline-core` -- the host already provides it. Marking it optional lets you typecheck in isolation.
...
- Importing host internals -- only import from `@sctg/cline-core`. Reaching into host-specific packages (e.g. CLI internals) will break in non-CLI hosts.
...
- (Package) `package.json` has `type: "module"`, `cline.plugins`, and `@sctg/cline-core` as an optional peer dep.
