...
`@sctg/cline-agents` (and by extension, the `Agent` class) is browser-safe with no Node.js dependencies. However, `@sctg/cline-core` and `ClineCore` require Node.js 22+. If you import from `@sctg/cline-sdk`, you get everything including the Node-only code. For browser usage, import directly from `@sctg/cline-agents`:
