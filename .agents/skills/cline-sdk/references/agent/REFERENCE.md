...
The `Agent` class (also exported as `AgentRuntime`) is the lightweight, stateless agent loop from `@sctg/cline-agents`. It handles the core iteration cycle: send messages to an LLM, execute tool calls, collect results, and repeat until the task is done.
