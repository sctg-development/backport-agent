...
`ClineCore` is the full-featured runtime from `@sctg/cline-core`. It wraps the `Agent` loop with session persistence, built-in tools (bash, editor, file reading, search, web fetch), config discovery, plugin loading, and optional hub-backed multi-process support.
...
```typescript
import { ClineCore } from "@sctg/cline-sdk"
