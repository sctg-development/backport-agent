---
name: cline-sdk
description: Comprehensive Cline SDK skill for building AI agents. Covers the Agent runtime, ClineCore sessions, custom tools, plugins, events, LLM providers, scheduling, multi-agent teams, and production deployment. Use for any task involving @sctg/cline-sdk or its sub-packages.
metadata:
...
1. Install with `npm install @sctg/cline-sdk`. The `@sctg/cline-sdk` package re-exports everything from `@sctg/cline-core`, `@sctg/cline-agents`, `@sctg/cline-llms`, and `@sctg/cline-shared`.
2. Requires Node.js 22 or later.
3. Use `createTool()` from `@sctg/cline-sdk` (or `@sctg/cline-shared`) to define tools. Tool names must be `snake_case`.
...
|   +-- agent/ (Agent class from @sctg/cline-agents)
+-- I need session persistence, built-in tools, config discovery
|   +-- clinecore/ (ClineCore from @sctg/cline-core)
...
| Package | Purpose |
|---------|---------|
| `@sctg/cline-sdk` | Everything you need, install this one |
| `@sctg/cline-core` | Sessions, persistence, built-in tools, config, hub |
| `@sctg/cline-agents` | Stateless agent loop, tool orchestration, streaming |
| `@sctg/cline-llms` | LLM provider gateway |
| `@sctg/cline-shared` | Types, tool helpers, hook engine |

## Resources

Repository: https://github.com/TEA-ching/cline
SDK Source: https://github.com/TEA-ching/cline/tree/main/sdk
Documentation: https://docs.ai-ml.pp.ua/sdk/overview
